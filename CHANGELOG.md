...
- **MCP tool argument aliases** — tools now accept `question`, `prompt`, and `text` as aliases for `message`; `buttons` and `choices` as aliases for `options`; `hint` as alias for `placeholder`. Fixes compatibility with agents that send non-standard but common parameter names
- **Tool descriptions clarified** — added explicit `PARAMETERS` section to each tool description listing exact parameter names from `inputSchema`, reducing ambiguity for AI agents
